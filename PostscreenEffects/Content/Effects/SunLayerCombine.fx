...
float4 PS_ExtractHDR(float2 inTex: TEXCOORD0) : COLOR0
{	
	float4 color = tex2D( gInputSampler, inTex );
	float lum = Luminance( color );
	if( lum >= gfThreshold )
	{
		float pos = lum - gfThreshold;
		if( pos > 0.98 ) pos = 0.98;
		if( pos < 0.02 ) pos = 0.02;
		color = tex2D( gGradientSampler, pos ) * gfBrightness;
...
technique Main
{
    pass p0
	{		
		PixelShader = compile ps_4_0 PS_ExtractHDR();	
    }
}